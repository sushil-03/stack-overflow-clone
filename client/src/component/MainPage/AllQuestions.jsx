...
const AllQuestions = ({type}) => {
    ...
    useEffect(()=>{
        ...
        if(!questions){
            return <div className='mt-10 text-4xl font-semibold font-sans w-full grid place-items-center'>
                    <h1 className='mb-14 shadow-2xl p-4 rounded-md'>Post some Question ....</h1>
            </div>
        }
  return (
    ...
    <div className=' overflow-scroll  '>
        {
            ...
            <div className='flex  md:w-1/5 w-1/4 items-center md:justify-center text-center md:gap-4 gap-2 text-sm md:flex-row flex-col justify-start ' >
                <span className='flex md:flex-col flex-row gap-2 items-center w-10'>
                   <span className='md:text-xl text-sm  font-semibold'>  {quest.votes}</span> <span className='text-sm '>votes</span>
                </span>
                <span className='flex md:flex-col flex-row gap-2 items-center w-10'>
                   <span className='md:text-xl text-sm  font-semibold'>  {quest.answers?.length}</span> <span className='text-sm '>answers</span>
                </span>
            </div>
            <div className='flex flex-col gap-1 relative flex-1'>
                <Link to={`/v1/question/${quest._id}`}>
                <span className='text-dk-blue md:text-lg text-sm   font-medium cursor-pointer'>
                    {quest.heading}
                </span>
                </Link>
                ...
                <span key={key} className='text-sky-800  lowercase px-2 py-1  rounded-sm text-xs ring-dk-blue bg-lt-blue/30 '>
                {tag}
                </span>
                    );
                        })
                ...
                <div className=''>
                    <span className="text-sky-800 text-xs absolute right-3">
